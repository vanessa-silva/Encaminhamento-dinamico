...
Após um \emph{Hello Packet} (cujo propósito é estabelecer conexões
...
Cada interface apenas envia informação sobre os destinos e redes
atingíveis por outras interfaces. FastEthernet 2/0 envia informações
sobre a rede 192.168.1.0/25, com métrica 2 (pois é calculada pelo
RIP). FastEthernet1/1 envia informações sobre os destinos alcançáveis
pela interface FastEthernet2/0, por exemplo 172.16.2.0 com métrica 6
(importada por OSPF).

\subparagraph{b.}
Este mecanismo chama-se ``split horizon'' - não anunciar numa
interface as rotas que aprendeu por ela.
